...
\subsection{核心思路}
我们使用\textbf{加时赛感知的 Bradley-Terry 配对比较模型}估计球队潜在实力参数，然后通过\textbf{校准逻辑回归}将实力差转化为胜率预测——5 折交叉验证 Brier 分数达到 \textbf{0.2410}（基线 0.2465）。
...
\midrule
基线（主队胜率 = 56.4\%） & 56.4\% & 0.6861 & 0.2465 \\
\textbf{加时赛感知 BT 逻辑回归} & \textbf{58.1\% $\pm$ 3.2\%} & \textbf{0.6752} & \textbf{0.2410} \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent\textbf{指标解读}：
\begin{itemize}
    \item \textbf{准确率}（58.1\%）：相比``永远预测主队胜''的基线（56.4\%）提升不大。这符合预期——冰球是四大职业联赛中随机性最高的运动，专业模型准确率也仅在 58--62\% 之间。
    \item \textbf{Log-Loss}（0.6752）：衡量概率预测的置信度质量。越低越好。
    \item \textbf{Brier 分数}（0.2410）：概率预测的均方误差。相比基线降低 2.2\%，在高随机性运动中已是有意义的提升。
\end{itemize}
...
\noindent 模型在 0.35--0.70 范围内校准良好。在高端（预测 0.75，实际 0.70）存在轻微\textbf{过度自信}，这是已知的局限性（见第~\ref{sec:limitations} 节）。
...
我们测试了四种备选权重方案：BT 独占（100\%）、均等（各 20\%）、xG 倾斜（xGD/60 占 50\%）。所有方案之间的 Spearman 秩相关系数为 \textbf{0.90--0.99}。前 6 名球队在所有方案中保持一致，证明排名对权重选择具有鲁棒性。
...
\textbf{关键校正}：原始 xG/60 受防守对位质量的混杂影响。面对精英防守组的一线自然会产出更少的 xG。我们通过\textbf{防守难度因子}进行归一化：

\begin{equation}
\text{校正 xG/60}_{\text{线}} = \frac{\text{原始 xG/60}_{\text{线}}}{\text{防守难度因子}_{\text{对位防守组}}}
\end{equation}

其中防守难度因子 = 该防守组的 xGA/60 与联盟平均值之比。
...
\begin{figure}[H]
    ...
    \caption{ML 模型对比仪表盘（6 面板）：（左上）5 折 CV 准确率对比——XGBoost 57.5\% 最高，（中上）Brier 分数与 Log-Loss 对比，（右上）各模型校准曲线，（左下/中下）Elastic Net 与 XGBoost 特征重要性，（右下）汇总表。总体上复杂 ML 仅带来边际提升，且可能牺牲校准；BT 仍是最重要的单一特征。}
    \label{fig:ml}
\end{figure}

\noindent\textbf{关键发现}：
\begin{itemize}[nosep]
    \item \textbf{最佳准确率}：XGBoost 达到 57.5\%，但 Log-Loss 0.7154、Brier 0.2575，校准明显变差，不适合作为概率输出模型。
    \item \textbf{最佳概率质量}：Elastic Net 逻辑回归在 Log-Loss（0.6774）与 Brier（0.2421）上最优，但准确率仅 57.0\%，且引入多变量复杂性。
    \item \textbf{可解释性最强且竞争力足够}：BT-only 逻辑回归（57.3\%，Log-Loss 0.6814，Brier 0.2437）已接近最优；混合集成（56.9\%）未带来稳定提升。
    \item XGBoost 和 Elastic Net 的特征重要性均显示 \textbf{BT 是最重要的单一预测变量}。
...
\begin{enumerate}
    ...
    \item \textbf{高端过度自信}：校准显示预测概率超过 70\% 时，实际偏乐观 5--13 个百分点（预测 0.75，实际 0.70）。折外保序回归 (isotonic calibration) 可缓解此问题。

    \item \textbf{冰球的固有随机性}：58.1\% 的准确率上限在冰球中是正常的。冰球是四大职业联赛中单场得分最低的运动，使得单场预测存在根本性限制。
...
\begin{itemize}
    ...
    \item \textbf{防守校正的进攻线差异指标}：控制了对位混杂因素。
