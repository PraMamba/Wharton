...
We fit an \textbf{overtime-aware Bradley-Terry paired comparison model} to estimate latent team strengths, then pass strength differentials through a \textbf{calibrated logistic regression} to produce win probabilities---achieving a 5-fold CV Brier score of \textbf{0.2430} (vs.\ 0.2459 baseline).
...
\textbf{OT-Aware BT Logistic} & \textbf{57.0\% $\pm$ 3.2\%} & \textbf{0.6797} & \textbf{0.2430} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Accuracy} (57.0\%): A small lift over always-predict-home (56.4\%) in random 5-fold CV; we interpret this as modest.
    \item \textbf{Log-Loss} (0.6797): Measures confidence quality. Lower = better-calibrated probabilities.
    \item \textbf{Brier Score} (0.2430): Probability MSE. The baseline reduction is about 1.2\% in random CV, so we treat it as a limited but useful calibration gain.
    \item \textbf{ID-order stress test (not true time)}: When sorting by numeric \texttt{game\_id}, BT underperforms the home-rate baseline on all three metrics (accuracy 54.7\% vs 56.2\%, Log-Loss 0.7047 vs 0.6860, Brier 0.2524 vs 0.2464). Because \texttt{game\_id} order in this simulated dataset is not guaranteed to be chronological, we treat this as a pessimistic robustness check rather than deployment-failure evidence.
\end{itemize}
...
To avoid unstable rates, we report only bins with at least 10 games.
...
\noindent The model is well-calibrated in the 0.44--0.69 range. There is \textbf{overconfidence at the high end} (predicted 0.79, observed 0.71), a known limitation noted in Section~\ref{sec:limitations}.
...
We tested four weight schemes (including the final submission weights): BT-only (100\%), equal weights (20\% each), and xG-heavy (50\% xGD/60), among others. Spearman rank correlations between schemes ranged from \textbf{0.81 to 1.00}. Five of the top~6 teams were shared across all schemes, indicating the top tier is broadly robust but not completely invariant to weighting. This is now reproducibly documented by the internal artifact \texttt{output/submission/ranking\_weight\_sensitivity.md} generated by \texttt{phase1\_submit.py}.
...
\begin{figure}[H]
    ...
    \caption{ML model comparison dashboard (6 panels): (top-left) 5-fold CV accuracy---Elastic Net leads at 58.0\%, (top-center) Brier score \& Log-Loss, (top-right) calibration curves, (bottom-left/center) Elastic Net and XGBoost feature importance, (bottom-right) summary table. Overall, complex ML can improve random-CV metrics slightly, but robustness and interpretability tradeoffs remain.}
    \label{fig:ml}
\end{figure}

\noindent\textbf{Key findings}:
\begin{itemize}[nosep]
    \item \textbf{Best random-CV accuracy}: Elastic Net reaches 58.0\% and is also best on probability quality (Log-Loss 0.6758, Brier 0.2412), but it adds multi-feature complexity.
    \item \textbf{XGBoost remains poorly calibrated}: accuracy 56.7\%, Log-Loss 0.7157, and Brier 0.2574 make it unsuitable for probability outputs despite its flexibility.
    \item \textbf{BT-only remains a strong, interpretable baseline}: OT-aware BT logistic (57.0\%, Log-Loss 0.6796, Brier 0.2430) is stable and transparent; the Blended Ensemble (57.8\%, Brier 0.2443) improves accuracy but not probability quality versus Elastic Net.
    \item Feature importance (from full-data refits, interpretive only) indicates \textbf{BT and win-rate-differential style summary strength signals} dominate; complex models mainly reweight these inputs.
...
\begin{enumerate}
    ...
    \item \textbf{High-end overconfidence}: Our calibration shows probabilities above 75\% are too optimistic by about 8 percentage points (predicted 0.79, observed 0.71). Post-hoc isotonic calibration on OOF predictions could mitigate this.

    \item \textbf{Hockey's inherent randomness}: Our 57.0\% CV accuracy is consistent with hockey's low signal-to-noise setting. The sport has the lowest score-per-game ratio among major sports, making individual game prediction fundamentally limited.
